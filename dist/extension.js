...
var NotionClient = class {
  ...
  constructor() {
    ...
    this.roadmapDbId = config.get("roadmapDatabaseId") || "";
    if (!token) {
      throw new Error("Notion token not configured. Set it in Settings > Extensions > Notion Agent");
    }
    this.client = new import_client.Client({ auth: token });
  }
  // Query roadmap tasks
  async getTasks(filter) {
    ...
    const response = await this.client.databases.query({
      ...
      filter: filterConditions.and.length > 0 ? filterConditions : void 0,
      sorts: [
  ...
  // Ask AI (simple keyword matching for MVP)
  async askAI(question) {
    const lowerQuestion = question.toLowerCase();
    if (lowerQuestion.includes("p0") || lowerQuestion.includes("today")) {
    ...
    if (lowerQuestion.includes("next")) {
      const tasks = await this.getTasks({ focus: "Next 3" });
      return this.formatTaskList(tasks, "Next 3 tasks");
    }
...
`;
    for (const task of tasks) {
      ...
      const url = task.url;
      result += `- **[${name}](${url})** (${priority}, ${effort}) - ${status}
`;
    }
    return result;
  }
  ...
  formatSearchResults(results) {
    ...
    for (const result of results) {
      const title = result.properties?.title?.title?.[0]?.plain_text || result.properties?.Task?.title?.[0]?.plain_text || "Untitled";
...
function activate(context) {
  const participant = vscode2.chat.createChatParticipant(
    ...
    async (request, context2, stream, token) => {
      try {
        ...
        const notionUri = vscode2.Uri.parse("https://notion.so");
        stream.reference(notionUri);
      } catch (error) {
        stream.markdown(`\u274C **Error**: ${error.message}

Make sure you've configured your Notion token in Settings > Extensions > Notion Agent.`);
      }
      return { metadata: { command: request.command } };
    }
  );
  participant.iconPath = vscode2.Uri.file(
    context.asAbsolutePath("media/icon.png")
  ).with({ scheme: "https" });
  context.subscriptions.push(participant);
}
async function handleTasksCommand(client, request, stream) {
  const prompt = request.prompt.toLowerCase();
  let filter = {};
  if (prompt.includes("p0")) {
    filter.priority = "P0";
  } else if (prompt.includes("p1")) {
    filter.priority = "P1";
  } else if (prompt.includes("p2")) {
    filter.priority = "P2";
  }
  ...
  const response = client.formatTaskList(tasks, "Filtered tasks");
  stream.markdown(response);
}
async function handleSearchCommand(client, request, stream) {
  const results = await client.search(request.prompt);
  const response = client.formatSearchResults(results);
  stream.markdown(response);
}
async function handleUpdateCommand(client, request, stream) {
  const match = request.prompt.match(/(?:update\s+)?([A-Z]+-\d+)\s+to\s+([A-Za-z ]+)/i);
  if (!match) {
    stream.markdown("\u274C **Format**: `update CVHC-123 to Done` or just `CVHC-123 to Done`");
    return;
  }
  ...
  try {
    ...
    stream.markdown(`\u274C Failed to update: ${error.message}`);
  }
}
function deactivate() {
}
